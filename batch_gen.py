...
class BatchGenerator(object):
    ...
    #             else:
    #                 print(f"Warning: Key '{content[i]}' not found in actions_dict")
    ...
    def next_batch(self, batch_size, if_warp=False): 
        ...
        mask = torch.zeros(len(batch_input), self.num_classes, max(length_of_sequences), dtype=torch.float)
        for i in range(len(batch_input)):
            input_shape = np.shape(batch_input[i])[1]
            target_shape = np.shape(batch_target[i])[0]
            # Get the shapes of the tensors
            input_shape = np.shape(batch_input[i])[1]
            target_shape = np.shape(batch_target[i])[0]

            # Check if the shapes match
            if input_shape != batch_input_tensor.size(2):
                # Resize the tensor
                batch_input[i] = np.resize(batch_input[i], (batch_input[i].shape[0], batch_input_tensor.size(2)))

            if target_shape != batch_target_tensor.size(1):
                # Resize the tensor
                batch_target[i] = np.resize(batch_target[i], (batch_target_tensor.size(1),))

            # Now the assignment should work
            batch_input_tensor[i, :, :input_shape] = torch.from_numpy(batch_input[i])
            batch_target_tensor[i, :target_shape] = torch.from_numpy(batch_target[i])
            mask[i, :, :target_shape] = torch.ones(self.num_classes, target_shape)
            ...
            # # if if_warp:
            #     warped_input, warped_target = self.warp_video(torch.from_numpy(batch_input[i]).unsqueeze(0), torch.from_numpy(batch_target[i]).unsqueeze(0))
            #     batch_input_tensor[i, :, :input_shape], batch_target_tensor[i, :target_shape] = warped_input.squeeze(0), warped_target.squeeze(0)
            # else:
            #     batch_input_tensor[i, :, :input_shape] = torch.from_numpy(batch_input[i])
            #     batch_target_tensor[i, :target_shape] = torch.from_numpy(batch_target[i])
            # mask[i, :, :target_shape] = torch.ones(self.num_classes, target_shape)
